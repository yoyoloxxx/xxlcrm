...
interface DataState { entities: EntityCfg[]; automations: Rule[]; routes: Route[]; records: Rec[]; tasks: Task[]; activities: Activity[]; chats: Chat[]; replyTemplates: ReplyTemplate[]; integrations: Integrations; users?: User[]; wsName?: string }
  ...
  load(): DataState | null {
    try {
      ...
      return {
        ...
        integrations: ints,
        // команда локального режима (своё имя, убранные демо-коллеги) и название компании
        users: Array.isArray(d.users) && d.users.length ? d.users : undefined,
        wsName: typeof d.wsName === "string" && d.wsName.trim() ? d.wsName : undefined,
      };
    } catch { return null; }
  },
  save(s: DataState) {
...
// Ведомая вкладка становится ведущей ТОЛЬКО через перечитывание базы. Раньше «Работать здесь»
// и автоповышение (ведущая уснула/закрылась) делали emit() → через 300 мс её УСТАРЕВШИЙ снимок
// уезжал в хранилище поверх всего, что успела записать соседка. Проверено: запись пропадала.
let noFlush = false;                                    // с этого момента вкладка ничего не пишет
function promote() {
  noFlush = true;
...
function tabBeat() {
  ...
  if (mine || !tabAlive(cur)) {
    try { window.localStorage.setItem(TAB_KEY, `${TAB_ID}|${Date.now()}`); } catch { /* ignore */ }
    if (tabFollower) promote();                         // ведущая ушла — перечитываем и продолжаем с её базой
    return;
  }
...
/** Стереть с устройства ключи каналов и личную переписку (общий ПК, «выйти и стереть»). */
export function wipeDeviceSecrets(): void {
  try { window.localStorage.removeItem(INT_KEY); window.localStorage.removeItem(PRIV_KEY); } catch { /* нет хранилища */ }
}
...
// записи, созданные в ЭТОЙ вкладке: только их пустые карточки убираем сами при закрытии
const createdHere = new Set<string>();
// Множественное число для названия раздела из единственного: «Заказ → Заказы», «Ученик → Ученики»,
// «Заявка → Заявки», «Объект → Объекты». Простые русские правила, человек всегда может поправить.
export function pluralGuess(one: string): string {
  const w = one.trim();
  if (!w) return "Раздел";
  const low = w.toLowerCase();
  if (/(ы|и)$/.test(low)) return w;                                   // уже множественное
  if (/[гкхжшчщ]а$/.test(low)) return w.slice(0, -1) + "и";          // книга → книги, задача → задачи
  if (/а$/.test(low)) return w.slice(0, -1) + "ы";                    // оплата → оплаты, работа → работы
  if (/я$/.test(low)) return w.slice(0, -1) + "и";                    // партия → партии
  if (/ь$/.test(low)) return w.slice(0, -1) + "и";                    // запись → записи
  if (/[гкхжшчщ]$/.test(low)) return w + "и";                         // ученик → ученики, платёж → платежи
  if (/(й)$/.test(low)) return w.slice(0, -1) + "и";                  // случай → случаи
  if (/[оеё]$/.test(low)) return w.slice(0, -1) + "а";                // окно → окна, дело → дела
  return w + "ы";                                                     // заказ → заказы, объект → объекты
}
...
const hadStored = !!loaded;
const initial: DataState = loaded ?? { entities: clone(ENTITIES), automations: defaultRules(), routes: defaultRoutes(), ...seed(), replyTemplates: DEFAULT_TEMPLATES, integrations: defaultIntegrations() };
...
const st: State = {
  ...
  users: initial.users?.some(u => u.id === "u1") ? initial.users : USERS, mode: "local", wsId: null,
  wsName: initial.wsName ?? "Digital Loft", inviteCode: "", authStage: null, pendingDraft: null, nav: null,
};
// Первый запуск: база из примеров ещё не записана — сохраняем сразу, иначе до первого действия
// «Копия базы» показывает 0 байт, а вторая вкладка не видит данных.
if (!hadStored) window.setTimeout(() => { try { persistence.save(st); } catch { /* места нет */ } }, 50);
...
/** Номер версии стора — для useMemo: сам объект st не меняется, меняется только его содержимое. */
...
// Телефон «как в карточке»: 8XXXXXXXXXX, 7XXXXXXXXXX, +7…, десять цифр с девятки → «+7 XXX XXX-XX-XX».
// Excel превращает номер в число и показывает «9,16E+09» — разворачиваем обратно в цифры.
// Иностранный или непонятный номер не трогаем (только обрезаем пробелы): лучше как есть, чем криво.
// Ключ склейки phoneKey от этого не зависит — он и раньше смотрел на последние десять цифр.
export function normalizePhone(raw: unknown): string {
  let s = String(raw ?? "").trim();
  if (!s) return s;
  const exp = /^(\d+)(?:[.,](\d+))?[eE]\+?(\d+)$/.exec(s);
  if (exp) {
    const n = Number(`${exp[1]}.${exp[2] ?? "0"}e${exp[3]}`);
    if (isFinite(n) && n > 0) s = String(Math.round(n));
  }
  const d = s.replace(/\D/g, "");
  let ten: string | null = null;
  if (d.length === 11 && (d[0] === "7" || d[0] === "8")) ten = d.slice(1);
  else if (d.length === 10 && d[0] === "9") ten = d;
  if (!ten) return s;
...
// Телефон как телефон: ссылки «позвонить / WhatsApp / Telegram». Только если это похоже
// на номер (≥10 цифр); «8 916 …» и «916 …» считаем Россией и приводим к +7 — иначе wa.me
// открывал бы чужую страну.
export function normalizePhoneView(v: unknown): { digits: string; tel: string; wa: string; tg: string } | null {
  let d = String(v ?? "").replace(/\D/g, "");
  if (d.length < 10 || d.length > 15) return null;
  if (d.length === 11 && d[0] === "8") d = "7" + d.slice(1);
  if (d.length === 10) d = "7" + d;
  return { digits: d, tel: `tel:+${d}`, wa: `https://wa.me/${d}`, tg: `https://t.me/+${d}` };
}
...
export const A = {
  openRecord(id: string | null) {
    ...
    // Только СВОЮ, созданную в этой вкладке: в облаке коллега мог нажать «+ Сделка» и отвлечься —
    // раньше её пустую карточку удалял любой, кто просто открыл и закрыл её.
    if (prev && prev !== id && createdHere.has(prev)) {
  ...
  moveStageAt(recId: string, stageId: string, beforeId: string | null) {
    const rec0 = recById(recId);
    const stage0 = rec0 && entityCfg(rec0.entityId).stages?.find(x => x.id === stageId);
    if (!rec0 || !stage0) return;
    const lostNow = stage0.kind === "lost" && rec0.stageId !== stageId;
    ...
    // Проигрыш без причины — слепая сводка. Поле «Причина отказа» заводим один раз (в том же
    // шаге отмены, что и перенос), открываем карточку и просим карточку подсветить это поле.
    // Перенос не блокируем: не ответил — так и останется пустым.
    if (lostNow) {
      const e = entityCfg(rec0.entityId);
      let reason = e.fields.find(f => f.type === "select" && /причин/i.test(f.label));
      if (!reason) {
        const labels = ["Дорого", "Передумал", "Не ответил", "Ушёл к конкуренту", "Другое"];
        const id = A.fieldAdd(e.id, {
          label: "Причина отказа", type: "select", inTable: false,
          options: labels.map((l, i) => ({ id: uid("o"), label: l, color: PALETTE[(i + 5) % PALETTE.length] })),
        });
        reason = entityCfg(e.id).fields.find(f => f.id === id);
        queueMicrotask(() => toast("Добавил поле «Причина отказа»", { description: "Укажите, почему сорвалось — сводка покажет, что чаще всего мешает" }));
      }
      if (reason) {
        const fieldId = reason.id;
        if (st.drawerRecordId !== recId) A.openRecord(recId);
        window.dispatchEvent(new CustomEvent("xxl:focus-field", { detail: { recId, fieldId } }));
      }
    }
  },
  createRecord(entityId: string, values: Record<string, unknown> = {}, stageId?: string, ownerId?: string): string {
    const __id = A.createRecordImpl(entityId, values, stageId, ownerId);
    createdHere.add(__id);
    return __id;
  },
  createRecordImpl(entityId: string, values: Record<string, unknown> = {}, stageId?: string, ownerId?: string): string {
    let id = "";
    mut(s => {
  ...
  // Кроме полей раздела mapping принимает псевдо-цели переезда из чужой CRM (см. IMPORT_TARGETS в csv.ts):
  // __stage, __created, __closed, __owner, __note, __extid, __relphone.
  importRecords(
    entityId: string,
    mapping: (string | null)[],
    rows: string[][],
    // stageMap: значение стадии из файла (в нижнем регистре, без лишних пробелов) → id стадии раздела,
    // либо «__new:open» / «__new:won» / «__new:lost» — завести стадию с таким названием и смыслом
    opts: { mergeByPhone?: boolean; stageId?: string; ownerId?: string; stageMap?: Record<string, string> } = {}
  ): {
    created: number; merged: number; badDates: number; unknownStages: string[]; unknownStageRows: number; stagesCreated: number;
    related: number; linkedByPhone: number; options: number; optionsCapped: boolean; ownersUnknown: number; notes: number;
  } {
    let created = 0, merged = 0, badDates = 0, related = 0, linkedByPhone = 0, options = 0, optionsCapped = false;
    let unknownStageRows = 0, stagesCreated = 0, ownersUnknown = 0, notes = 0;
    const unknownStages = new Set<string>();
    const OPTION_CAP = 60;      // 3 000 значений из файла превращали конфиг раздела в кирпич
    const T = { stage: "__stage", created: "__created", closed: "__closed", owner: "__owner", note: "__note", extId: "__extid", relPhone: "__relphone" };
    ...
    mut(s => {
      ...
      // «ID в старой системе»: скрытое поле заводим только когда колонка с ID реально сопоставлена.
      // По нему повторная загрузка того же файла дополняет записи, а не плодит дубли.
      const usesExt = mapping.includes(T.extId);
      if (usesExt && !e.fields.some(f => f.id === "ext_id")) e.fields.push({ id: "ext_id", label: "ID в старой системе", type: "text", inTable: false });
      const numOf = new Map<string, number>();
      for (const r of s.records) numOf.set(r.entityId, (numOf.get(r.entityId) ?? 0) + 1);
      const nextNum = (entId: string) => { const n = (numOf.get(entId) ?? 0) + 1; numOf.set(entId, n); return n; };
      // Указатели «телефон → запись», «ID → запись» и «название → запись» строим ОДИН раз.
      ...
      const byExt = new Map<string, Rec>();
      if (usesExt) for (const r of s.records) {
        if (r.entityId !== entityId) continue;
        const x = String(r.values["ext_id"] ?? "").trim();
        if (x && !byExt.has(x)) byExt.set(x, r);
      }
      // ключ по названию — без учёта регистра и лишних пробелов: «Иванов  Пётр» из amo = «иванов пётр»
      const byTitle = new Map<string, Rec>();
      const titleKey = (entId: string, v: unknown) => entId + "\u0000" + norm(v);
      for (const r of s.records) {
      ...
      // телефоны в справочниках-целях связей (строим лениво, только если связь реально ищется по телефону)
      const relPhoneIdx = new Map<string, Map<string, Rec>>();
      const relPhoneIndex = (ent: EntityCfg, pf: Field) => {
        let m = relPhoneIdx.get(ent.id);
        if (!m) {
          m = new Map<string, Rec>();
          for (const r of s.records) {
            if (r.entityId !== ent.id) continue;
            const d = phoneKey(r.values[pf.id]);
            if (d && !m.has(d)) m.set(d, r);
          }
          relPhoneIdx.set(ent.id, m);
        }
        return m;
      };
      // комментарии, которые у записи уже есть: повторная загрузка не должна дублировать примечания
      const commentsOf = new Map<string, Set<string>>();
      for (const a of s.activities) {
        if (a.kind !== "comment") continue;
        let set = commentsOf.get(a.recordId);
        if (!set) { set = new Set(); commentsOf.set(a.recordId, set); }
        set.add(a.text);
      }
      const addComment = (recId: string, text: string, ts: number): boolean => {
        const t = text.length > ACT_MAX ? text.slice(0, ACT_MAX) + "…" : text;
        let set = commentsOf.get(recId);
        if (!set) { set = new Set(); commentsOf.set(recId, set); }
        if (set.has(t)) return false;
        set.add(t);
        s.activities.push({ id: uid("a"), recordId: recId, ts, kind: "comment", text: t, userId: s.currentUserId });
        return true;
      };
      // «Ответственный» из файла → сотрудник: полное совпадение имени, иначе общее слово (имя или фамилия):
      // «Марина Петрова» из amo находит «Марину», «Петрова М.» — тоже. Подстроки нарочно не ищем:
      // «Глебова Анна» не должна становиться «Глебом».
      const ownerCache = new Map<string, string | undefined>();
      const findUser = (raw: string): string | undefined => {
        const q = norm(raw);
        if (!q) return undefined;
        if (ownerCache.has(q)) return ownerCache.get(q);
        const words = q.split(" ").filter(w => w.length >= 2);
        const u = s.users.find(x => norm(x.name) === q)
          ?? s.users.find(x => { const w = norm(x.name).split(" "); return words.some(y => w.includes(y)); });
        ownerCache.set(q, u?.id);
        return u?.id;
      };
      // стадии: таблица соответствий из диалога → по названию → «не нашли» (первая стадия, об этом скажем)
      const stageMap = opts.stageMap ?? {};
      const madeStages = new Map<string, string>();
      const makeStage = (label: string, kind: Stage["kind"]): string => {
        if (!e.stages) e.stages = [];
        const stage: Stage = { id: uid("s"), label: label.trim() || "Стадия", color: PALETTE[e.stages.length % PALETTE.length], kind };
        // рабочая стадия встаёт перед финальными (как в конструкторе), финальная — в конец
        const firstFinal = e.stages.findIndex(x => x.kind !== "open");
        if (kind === "open" && firstFinal !== -1) e.stages.splice(firstFinal, 0, stage); else e.stages.push(stage);
        stagesCreated++;
        return stage.id;
      };
      const resolveStage = (raw: string): string | undefined => {
        if (!e.stages?.length) return undefined;
        const key = norm(raw);
        const mapped = stageMap[key];
        if (mapped?.startsWith("__new")) {
          let id = madeStages.get(key);
          if (!id) {
            const kind = mapped.endsWith(":won") ? "won" : mapped.endsWith(":lost") ? "lost" : "open";
            id = makeStage(raw, kind);
            madeStages.set(key, id);
          }
          return id;
        }
        if (mapped && e.stages.some(x => x.id === mapped)) return mapped;
        return e.stages.find(x => norm(x.label) === key)?.id;
      };
      // связь, к которой относится «телефон клиента», если колонку с именем клиента не сопоставили
      const relByPhoneOnly = e.fields.find(f => {
        if (f.type !== "relation") return false;
        const t = s.entities.find(x => x.id === f.relationTo);
        return !!t && !t.stages?.length && t.fields.some(ff => ff.type === "phone");
      });

      for (const row of rows) {
        const values: Record<string, unknown> = {};
        let stageId = opts.stageId ?? e.stages?.[0]?.id;
        let createdAt: number | undefined, closedAt: number | undefined;
        let ownerRaw = "", extId = "", relPhoneRaw = "";
        const rowNotes: string[] = [];
        const relRaw: [Field, string][] = [];
        mapping.forEach((target, i) => {
          const raw = (row[i] ?? "").trim();
          if (!target || !raw) return;
          switch (target) {
            case T.stage: {
              const sid = resolveStage(raw);
              if (sid) stageId = sid;
              else { unknownStages.add(raw); unknownStageRows++; }   // «Отказ» из чужой CRM не должен молча уехать в работу
              return;
            }
            case T.created: { const ts = parseRu(raw, undefined, true); if (ts) createdAt = ts; else badDates++; return; }
            case T.closed: { const ts = parseRu(raw, undefined, true); if (ts) closedAt = ts; else badDates++; return; }
            case T.owner: ownerRaw = raw; return;
            case T.note: rowNotes.push(String(cleanText(raw))); return;
            case T.extId: extId = raw.slice(0, 80); return;
            case T.relPhone: relPhoneRaw = raw; return;
          }
          ...
          switch (f.type) {
            ...
            case "date": case "datetime": {
              // «12.03.2025 14:22» из выгрузки: для даты — полдень дня, для даты-времени — точное время
              const ts = parseRu(raw, undefined, f.type === "datetime");
            ...
            case "relation": relRaw.push([f, raw]); break;   // разбираем после строки: телефон клиента может идти позже
            case "phone": values[f.id] = normalizePhone(cleanText(raw)); break;
            case "text": case "textarea": {
              // несколько колонок в одно текстовое поле — «Имя» + «Отчество» + «Фамилия» склеиваются через пробел
              const v = String(cleanText(raw));
              const prev = values[f.id];
              values[f.id] = prev ? `${String(prev)} ${v}` : v;
              break;
            }
            default: values[f.id] = cleanText(raw);
          }
        });
        // связь с клиентом: по телефону из этой же строки → по имени (без регистра и лишних пробелов) →
        // заводим карточку с именем И телефоном (раньше — только с именем, и телефон клиента терялся)
        const relPhoneNorm = relPhoneRaw ? normalizePhone(cleanText(relPhoneRaw)) : "";
        const relPhoneK = phoneKey(relPhoneNorm);
        const relTargets: [Field, string][] = relRaw.length ? relRaw : relPhoneK && relByPhoneOnly ? [[relByPhoneOnly, ""]] : [];
        const rowOwner = ownerRaw ? findUser(ownerRaw) : undefined;
        for (const [f, raw] of relTargets) {
          const target2 = s.entities.find(x => x.id === f.relationTo);
          if (!target2) continue;
          const tPhoneF = target2.fields.find(x => x.type === "phone");
          let found: Rec | undefined;
          if (tPhoneF && relPhoneK) { found = relPhoneIndex(target2, tPhoneF).get(relPhoneK); if (found) linkedByPhone++; }
          if (!found && raw) found = byTitle.get(titleKey(target2.id, raw));
          if (found) {
            // у найденного по имени клиента нет телефона, а в файле есть — дописываем
            if (tPhoneF && relPhoneK && isBlank(found.values[tPhoneF.id])) { found.values[tPhoneF.id] = relPhoneNorm; relPhoneIndex(target2, tPhoneF).set(relPhoneK, found); }
            values[f.id] = found.id;
            continue;
          }
          if (!raw && !relPhoneNorm) continue;
          const at = createdAt ?? now();
          const nr: Rec = {
            id: uid("r"), entityId: target2.id, num: nextNum(target2.id),
            values: { [target2.titleFieldId]: raw || relPhoneNorm }, ownerId: rowOwner ?? opts.ownerId ?? s.currentUserId,
            createdAt: at, updatedAt: at, stageId: target2.stages?.[0]?.id, stageAt: at, pos: 1000,
          };
          if (tPhoneF && relPhoneNorm) nr.values[tPhoneF.id] = relPhoneNorm;
          s.records.push(nr);
          if (raw) byTitle.set(titleKey(target2.id, raw), nr);
          if (tPhoneF && relPhoneK) relPhoneIndex(target2, tPhoneF).set(relPhoneK, nr);
          s.activities.push({ id: uid("a"), recordId: nr.id, ts: now(), kind: "created", text: "Заведён при загрузке из файла", userId: s.currentUserId });
          related++;                            // об этом скажем в отчёте: файл на 300 строк заводил 600 записей
          values[f.id] = nr.id;
        }
        if (!Object.keys(values).length) continue;
        if (extId) values["ext_id"] = extId;
        const ownerMissing = !!ownerRaw && !rowOwner;
        const noteTs = createdAt ?? now();
        // дедуп: тот же ID из старой CRM или тот же телефон — дополняем существующую запись, а не плодим вторую
        const rowKey = phoneF ? phoneKey(values[phoneF.id]) : null;
        const dup = (extId ? byExt.get(extId) : undefined) ?? (opts.mergeByPhone && rowKey ? byPhone.get(rowKey) : undefined);
        if (dup) {
          for (const [k, v] of Object.entries(values)) if (dup.values[k] === undefined || dup.values[k] === "") dup.values[k] = v;
          dup.updatedAt = now();
          if (extId && !byExt.has(extId)) byExt.set(extId, dup);
          for (const n of rowNotes) if (addComment(dup.id, n, noteTs)) notes++;
          if (ownerMissing) { ownersUnknown++; addComment(dup.id, `Ответственный в старой CRM: ${ownerRaw}`, noteTs); }
          merged++;
          continue;
        }
        // Даты из старой CRM: создана — как в файле (сводка «новых за неделю» не считает переезд новыми
        // заявками); закрыта — момент попадания в стадию. Без «закрыта» у финальной стадии берём дату
        // создания (нижняя оценка — иначе весь старый архив стал бы «выиграно на этой неделе»),
        // у рабочей — сейчас, как раньше: иначе правило «застряла» завалило бы задачами по всему архиву.
        const cAt = createdAt ?? now();
        const kind = e.stages?.find(x => x.id === stageId)?.kind ?? "open";
        const sAt = closedAt ?? (kind === "open" ? now() : cAt);
        const r: Rec = {
          id: uid("r"), entityId, num: nextNum(entityId), values,
          ownerId: rowOwner ?? opts.ownerId ?? s.currentUserId, createdAt: cAt, updatedAt: Math.max(cAt, closedAt ?? cAt),
          stageId, stageAt: sAt, pos: (numOf.get(entityId) ?? 0) * 1000 + 1000,
        };
        s.records.push(r);
        // указатели пополняем сразу: иначе два одинаковых телефона ВНУТРИ одного файла
        // не склеивались — «объединять по телефону» работало только со старой базой
        if (rowKey && !byPhone.has(rowKey)) byPhone.set(rowKey, r);
        if (extId && !byExt.has(extId)) byExt.set(extId, r);
        s.activities.push({ id: uid("a"), recordId: r.id, ts: now(), kind: "created", text: "Загружено из файла", userId: s.currentUserId });
        for (const n of rowNotes) if (addComment(r.id, n, noteTs)) notes++;
        // сотрудника с таким именем в команде нет — запись достаётся выбранному в диалоге, а имя не теряем
        if (ownerMissing) { ownersUnknown++; addComment(r.id, `Ответственный в старой CRM: ${ownerRaw}`, noteTs); }
        created++;
      }
    });
    if (created || merged) markSetup("imported");
    return {
      created, merged, badDates, unknownStages: [...unknownStages].slice(0, 5), unknownStageRows, stagesCreated,
      related, linkedByPhone, options, optionsCapped, ownersUnknown, notes,
    };
  },
  ...
  taskAddAt(id: string | null, title: string, kind: Task["kind"], dueTs: number, recId?: string): boolean {
    ...
    mut(s => {
      ...
      s.tasks.push({ id: id ?? uid("t"), title, kind, recordId: recId, ownerId: r?.ownerId ?? s.currentUserId, due: dueTs, done: false, demo: r?.demo || undefined });
  ...
  resetDemo() {
    ...
    // Демо-коллеги «Марина» и «Артём» — тоже пример: в локальном режиме команды нет, есть один человек
    const demoUsers = st.users.filter(u => u.id !== st.currentUserId);
    if (!demoRecs.length && !demoChats.length && !demoTasks.length && !demoUsers.length) {
    ...
    mut(s => {
      ...
      if (s.activeChatId && !s.chats.some(c => c.id === s.activeChatId)) s.activeChatId = null;
      // остаётся только текущий пользователь; осиротевшие записи и задачи — ему
      const me = s.currentUserId;
      s.users = s.users.filter(u => u.id === me);
      for (const r of s.records) if (r.ownerId !== me) { r.ownerId = me; }
      for (const t of s.tasks) if (t.ownerId !== me) { t.ownerId = me; }
      repairStructure(s); // маршруты «по очереди»/на удалённого сотрудника — к владельцу
    });
    toast(demoRecs.length ? `Примеры убраны: ${demoRecs.length} ${plural(demoRecs.length, "запись", "записи", "записей")}` : "Демо-коллеги убраны — в команде только вы", {
      description: demoRecs.length ? "Ваши записи, диалоги и подключения на месте, демо-коллеги убраны. Ctrl+Z вернёт примеры" : "Записи и задачи переназначены на вас",
    });
  },
  ...
  // «Чужая личность»: в демо человек входит как «Глеб». Переименовать себя можно сразу,
  // не дожидаясь облака (там имя живёт в профиле пространства — этот путь не для него).
  renameMe(name: string) {
    const n = String(cleanText(name) ?? "").trim().slice(0, 60);
    if (!n || st.mode !== "local") return;
    mut(s => { s.users = s.users.map(u => (u.id === s.currentUserId ? { ...u, name: n } : u)); });
  },
  ...
  entAdd(name: string, withPipeline: boolean, namePlural?: string): string {
    const id = uid("e");
    const titleId = uid("f");
    // «Ответственный» у записи есть всегда (владелец записи) — второе одноимённое поле давало
    // две одинаковые колонки в новом разделе. Множественное число — своё, а не копия единственного.
    const one = name.trim() || "Запись";
    const many = (namePlural ?? "").trim() || pluralGuess(one);
    const ent: EntityCfg = {
      id, name: one, namePlural: many, icon: "folder", titleFieldId: titleId,
      fields: [
        { id: titleId, label: "Название", type: "text", inTable: true, required: true },
        { id: uid("f"), label: "Телефон", type: "phone", inTable: true },
...
export function handleIncoming(ext: ChatExt, name: string, channel: Channel, text: string, phone?: string) {
  ...
  // ЛИЧНЫЙ Telegram: диалоги остаются на устройстве, и заявка из них заводится ТОЛЬКО рукой
  // («Это клиент»). Раньше маршрут auto превращал маму и друзей в карточки клиентов, сделки и
  // задачи в общем пространстве — с именем и телефоном, видимыми всей команде.
  const priv = ext.tgu !== undefined;
  const found = st.chats.find(c => c.ext && chatExtMatch(c.ext, ext));
  if (found) {
    A.chatIncoming(found.id, text);
    if (priv && !found.shared) return;
  ...
  if (!priv && routeOf(channel).auto) A.chatCreateLead(id); // маршрут канала решает: сразу заявка или только диалог
}
